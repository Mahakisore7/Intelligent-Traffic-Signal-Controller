# evaluate_q_agent.py
# This script is for TAKING THE TEST.
# It loads the pre-trained Q-Table and runs the simulation with no random actions.
...
if __name__ == "__main__":
    ...
    print("Starting evaluation of the trained Q-Learning agent...")
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        # 2. CHOOSE ACTION (EXPLOITATION ONLY, NO RANDOMNESS)
        if current_state in q_table:
            action = np.argmax(q_table[current_state])
        else:
            # If the agent encounters a state it never saw during training,
            # it will take a default safe action (staying).
            action = ACTION_STAY
