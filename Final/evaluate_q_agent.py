# evaluate_q_agent.py
# This script loads a trained Q-Table and runs a single simulation
# to evaluate its performance. It uses the GUI and does not perform exploration.
...
if __name__ == "__main__":
    # Command to run with GUI and generate the final tripinfo report for this agent
    ...
    print("Starting evaluation of trained Q-Learning Agent...")
    
    while traci.simulation.getMinExpectedNumber() > 0:
        current_state = get_state()

        # Check if the agent has a learned policy for this state
        if current_state in q_table:
            # Exploit: Choose the best action from the Q-Table with no randomness
            action = np.argmax(q_table[current_state])
        else:
            # If the state is new/unknown, default to a safe action (stay)
            action = ACTION_STAY

        # Perform the chosen action in the simulation
        current_phase = traci.trafficlight.getPhase(TLS_ID)
    ...
    traci.close()
    print("Evaluation finished. Results saved to 'tripinfo_q_learning.xml'.")